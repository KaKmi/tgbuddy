  ...
  return [
    ...
    ...(mode === 'plan'
      ? [
          ...
          '调查文件时优先使用 glob、grep、read；glob 返回空结果只表示未发现匹配文件，不要再拼接 ls/dir 兼容命令探测，必要时直接询问用户。',
          '计划完成后直接调用 exit_plan_mode 提交计划审批；普通对话中的“无需调整”不代表已经批准。',
          '批准前不得尝试执行副作用。批准后只执行审批清单内的效果。',
          '工具返回失败时，不得声称已经完成、已经写入或已经修改；必须依据真实 Tool 结果报告。',
        ]
      : []),
    ...(skills.length > 0
      ? [
