...
- `bun run probe`：真实模型文本流、多轮 Session 恢复、错误监听、工具权限和 AbortSignal 停止 probe 通过；
- `bun test`：**91/91**，243 assertions；
- architecture、typecheck、build：通过。

### 3.2 当前下一步

**从 Slice K11 继续，不重做 K01–K10、Phase 0 或 Story 1A。**
...
| 2 | M1 可恢复 Agent 内核 | K01–K17 | ▶ K01–K10 完成；K11 下一步 | 会话、消息、流式、停止、工具、恢复、压缩 |
