...
这是当前批准的目标架构。现有代码已经实现了 Electron IPC、pi Agent、权限挂起、计划工具和 JSONL 会话的早期闭环，但以下目标仍未全部落地：SQLite canonical storage、Workspace/WorkspaceMount、BlobStore、MCP Manager、完整 Harness 生命周期和单层 child delegation。因此文档状态为 `partially-outdated`，表示设计已收敛而实现仍在迁移中。
...
| Agent 生命周期 | 目标使用 pi `AgentHarness`；当前 `src/main/orchestrator.ts` 的裸 `Agent` 仅视为迁移中的实现 |
...
目录移动或重新挂载只更新 `WorkspaceMount.rootPath` 和状态，不改变 `workspaceId`、Session、权限规则、Plan 或产物归属。当前 `orchestrator.ts` 的 `resolveWorkspace()` 仍会在 `~/.tgbuddy/workspaces/{id}` 自动创建目录，这是迁移前的临时实现，不能作为最终模型。
...
`AgentHarness` 已提供 Session、JSONL/Memory backend、compaction、Skills、Prompt Templates、Tool hooks、steering/follow-up queue、save point 和 settled 生命周期。TgBuddy 应复用这些能力，并在其外部接入 SQLite、PolicyEngine、MCP、BlobStore 和 Electron IPC。当前 `src/main/orchestrator.ts` 直接 `new Agent()` 的实现会自行管理上述生命周期，属于必须标记清楚的迁移债务。
...
| 当前实现 | 目标修正 |
|---|---|
| `src/main/orchestrator.ts` 直接 `new Agent()` | 迁移到 `AgentHarness`，由 Harness 管 save point、settled、Session context 和 tool lifecycle |
| `src/main/session-store.ts` 使用 `sessions.json` + JSONL | 先做 SQLite spike，通过后由 `SqliteSessionRepo` 作为 canonical；保留 JSONL importer/exporter |
...
| `newId()` 为 8 位 hex | 改为 `uuidv7` 或至少 128-bit ID，避免跨 Run、Blob、ToolCall 碰撞 |
